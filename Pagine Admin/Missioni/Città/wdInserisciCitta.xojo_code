#tag WebPage
Begin WebDialog wdInserisciCitta
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   90
...
#tag WindowCode
	#tag Event
		Sub Close()
		  if Session.Available then
		    Session.wdInserisciCittaAperto = False
		  end if
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub Open()
		  if Session.Available then
		    Session.wdInserisciCittaAperto = True
		  end if
	...
	#tag Event
		Sub Pressed()
		  ...
		  if connettiDB then
		    ...
		    sql = "INSERT INTO town(name) VALUES ('"+Apici(txtNome.Text)+"')"
		    db.SQLExecute(sql)
		    if db.Error Then
