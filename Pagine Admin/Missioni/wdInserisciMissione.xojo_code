#tag WebPage
Begin WebDialog wdInserisciMissione
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   703
...
#tag WindowCode
	#tag Event
		Sub Close()
		  if Session.Available then
		    Session.wdInserisciMissioneAperto = False
		  end if
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub Open()
		  if Session.Available then
		    Session.wdInserisciMissioneAperto = True
		  end if
...
#tag Events cmdCrea
	#tag Event
		Sub Pressed()
		  ...
		  //Se le missioni non sono con Favor Point controlla il resto
		  if pmTipo.RowTag(pmTipo.ListIndex) <> 7 then
		    ...
		    if txtEnergiaScaricata.Text = "" then
		      MsgBox "Insert the maximum experience."
		    ...
		    if txtCashPremio.Text="" AND txtSkillPoint.Text="" AND txtHG.Text="" AND txtEsperienza.Text="" Then
		  ...
		  //Inserisco nel db
		  dim sql As String
		  sql = "INSERT INTO mission(title, subtitle, mobster_required, energy_required, idtown, cash_min, cash_max, experience, energy_discharge, Visible, level_required, idgroup, favor_point_required, skill_point, hired_guns, cash_award, skill_point_award, hired_guns_award, experience_award) VALUES ('" + Apici(txtNome.Text)
		  sql = sql + "','"+Apici(txtSottotitolo.Text) + "', '" + txtMobRichiesti.Text + "','" + txtEnergiaRichiesta.Text + "','" + pmCittà.RowTag(pmCittà.ListIndex) + "','"+RimuoviFormattazioneNumeri(txtCashMin.Text) + "','"
		  sql = sql + RimuoviFormattazioneNumeri(txtCashMax.Text) + "','" + txtEsperienza.Text + "','" + txtEnergiaScaricata.Text + "','" + str(cbVisibile.Value)+ "','"+txtLivello.Text+"','"+str(idCategoriaMadre)+"','"+numFavorPoint+"','"+txtSkillPoint.Text+"','"+txtHG.Text+"','"+txtCashPremio.Text+"','"+txtSkillPointPremio.Text+"','"+txtHGPremio.Text+"','"+txtEsperienzaPremio.Text+"')"
		  ...
		  if db.Error Then
		    ...
		    Exit sub
		  Else
		    idmissione=db.LastRowID
		  end if
		  
		  //Inserimento informazioni per quanto riguarda i tipi
		  sql = "INSERT INTO relation_type_mission(idmission, idtype) VALUES("+str(idmissione)+","+pmTipo.RowTag(pmTipo.ListIndex)+");"
		  db.SQLExecute(sql)
		  if db.Error Then
		    MsgBox ErroreDatabase
		    db.Rollback
		  ...
		  //Inserisco le date per le missioni a tempo
		  if pmTipo.RowTag(pmTipo.ListIndex)=1 then
		    sql = "INSERT INTO mission_validation(idmission, valid_from, valid_to) VALUES("+str(idmissione)+",'"+TornaDataSQL(txtValidoDa.Text)+" "+TornaOraSQL(txtOraValidoDa.Text)+"','"+TornaDataSQL(txtValidoA.Text)+" "+TornaOraSQL(txtOraValidoA.Text)+"')"
		    db.SQLExecute(sql)
		    if db.Error Then
		      MsgBox ErroreDatabase
		      db.Rollback
		  ...
		  MsgBox "The mission has been created with successful."
	...
	#tag Event
		Sub Pressed()
		  if Session.wdArmiRichiesteAperto=False then
		    ...
		    t.lstwdArmiRichieste = self.lstArmiRichieste
