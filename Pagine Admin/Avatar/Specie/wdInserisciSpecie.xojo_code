#tag WebPage
Begin WebDialog wdInserisciSpecie
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   82
...
#tag WindowCode
	#tag Event
		Sub Close()
		  if Session.Available then
		    Session.wdInserisciSpecieAperto = False
		  end if
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub Open()
		  if Session.Available then
		    Session.wdInserisciSpecieAperto = True
		  end if
...
#tag Events cmdCrea
	#tag Event
		Sub Pressed()
		  ...
		  if connettiDB Then
		    dim sql As String
		    sql = "INSERT INTO species(name) VALUES('" + Apici(txtNome.Text) + "')"
		    ...
		    MsgBox "The species has been insereted with successful."
