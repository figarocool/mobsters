#tag WebPage
Begin WebDialog wdModificaAvatar
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   521
...
#tag WindowCode
	#tag Event
		Sub Close()
		  if Session.Available then
		    Session.wdModificaAvatarAperto = False
		  end if
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub Open()
		  if Session.Available then
		    Session.wdModificaAvatarAperto = True
		  end if
	...
	#tag Event
		Sub Shown()
		  if connettiDB then
		    dim sql As String
		    //Faccio la SELECT per l'avatar e il JOIN per trovare la specie
		    sql = "SELECT avatar.name, image_path, image_name, species.idspecies FROM avatar" + EndOfLine
		    sql = sql + "INNER JOIN species" + EndOfLine
		    sql = Sql + "ON species.idspecies=avatar.idspecies" + EndOfLine
		    sql = sql + "WHERE avatar.idavatar="+lstwdAvatar.RowTag(lstwdAvatar.ListIndex)
		    dim rs As RecordSet
		    rs = db.SQLSelect(sql)
		    if db.Error then
		    ...
		    //Completo i vari campi
		    txtNome.Text = DeApici(rs.IdxField(1).StringValue)
		    
		    //Seleziono l'immagine e poi la carico in una Picture
		    immaginePrecedente = dir.Child("upload").Child(rs.IdxField(2).StringValue).Child(DeApici(rs.IdxField(3).StringValue))
		    ...
		    dim i As Integer
		    //Seleziono la specie
		    for i = 0 to pmSpecie.ListCount-1
		      if pmSpecie.RowTag(i)=rs.IdxField(4).Value then
		        pmSpecie.ListIndex = i
		      end if
		    next
	...
	#tag Event
		Sub Pressed()
		  ...
		  if connettiDB then
		    ...
		    dim sql As String
		    sql = "UPDATE avatar SET name='"+Apici(txtNome.Text)+"', image_path='"+CartellaUpload.Name+"', image_name='"+Apici(immagineCorrente.Name)+"', idspecies="+pmSpecie.RowTag(pmSpecie.ListIndex)+" WHERE idavatar="+lstwdAvatar.RowTag(lstwdAvatar.ListIndex)
		    db.SQLExecute(sql)
		    if db.Error then
		      MsgBox ErroreDatabase
		      exit sub
		    ...
		    MsgBox "The avatar has been modified with successful."
