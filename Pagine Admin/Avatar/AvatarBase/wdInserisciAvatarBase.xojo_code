#tag WebPage
Begin WebDialog wdInserisciAvatarBase
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   452
...
#tag WindowCode
	#tag Event
		Sub Close()
		  if Session.Available then
		    Session.wdInserisciAvatarBaseAperto = False
		  end if
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub Open()
		  if Session.Available then
		    Session.wdInserisciAvatarBaseAperto = True
		  end if
	...
	#tag Method, Flags = &h0
		Sub AggiungiEquipaggiamento()
		  ...
		  if connettiDB then
		    ...
		    sql = sql + "WHERE idavatar="+pmAvatar.RowTag(pmAvatar.ListIndex) + EndOfLine
		    ...
		    While Not rs.EOF
		      ...
		      lstEquipaggiamento.RowTag(lstEquipaggiamento.LastIndex)=rs.IdxField(2).Value
		      rs.MoveNext
		    Wend
...
#tag Events cmdRimuovi
	#tag Event
		Sub Pressed()
		  ...
		  'end if
		  '
		  'imAvatar.EliminaImmagine(lstEquipaggiamentoSelezionato.RowTag(lstEquipaggiamentoSelezionato.ListIndex))
		  '
		  'lstEquipaggiamento.AddRow(lstEquipaggiamentoSelezionato.Cell(lstEquipaggiamentoSelezionato.ListIndex, 0))
		  'lstEquipaggiamento.Cell(lstEquipaggiamento.LastIndex, 1)=lstEquipaggiamentoSelezionato.Cell(lstEquipaggiamentoSelezionato.ListIndex, 1)
		  'lstEquipaggiamento.RowTag(lstEquipaggiamento.LastIndex) = lstEquipaggiamentoSelezionato.RowTag(lstEquipaggiamentoSelezionato.ListIndex)
...
#tag Events imAvatar
	#tag Event
		Sub RichiestaSalvataggioDati(jsonPosizioni As String)
		  ...
		  //Faccio update delle immagini dell'equipaggiamento
		  if connettiDB Then
		  ...
		  for i as integer=0 to UBound(me.images)
		    dim sql As String
		    sql="INSERT INTO basic_avatar(idavatar, id_avatar_equipment, id_equipment) VALUES("+pmAvatar.RowTag(pmAvatar.ListIndex)+", "+str(me.images(i).ID)+", "+str(me.images(i).IDequipaggiamento)+")"
		    db.SQLExecute(sql)
		    if db.Error then
		      MsgBox ErroreDatabase
		      db.Rollback
		  ...
		  AggiornalstAvatarBase(lstwdAvatarBase)
		  MsgBox "Basic avatar has been created."
		  self.Close
		End Sub
	#tag EndEvent
	#tag Event
...
#tag Events pmAvatar
	...
	#tag Event
		Sub Opening()
		  ...
		  //Riempo la Listbox dell'equipaggiamento selezionato
		  if connettiDB then
		    dim sql As String
		    ...
		    sql = sql + "WHERE basic_avatar.id ISNULL"+EndOfLine
