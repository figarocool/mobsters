...
Begin WebPage wpAvatarBase
   ...
   Begin WebSearchField txtRicerca
      ...
      _mPanelIndex    =   -1
   End
   Begin WebListBox lstAvatarBase
      ColumnCount     =   2
      ColumnWidths    =   "15%, *"
      ControlID       =   ""
      Enabled         =   True
	...
	#tag Event
		Sub Pressed()
		  ...
		  if connettiDB Then
		    ...
		    sql = sql + "WHERE basic_avatar.idavatar LIKE '%"+Apici(txtRicerca.Text)+"%' OR name LIKE '%"+Apici(txtRicerca.Text)+"%'"+EndOfLine
...
#tag Events cmdVisualizzaTutto
	#tag Event
		Sub Pressed()
		  AggiornalstAvatarBase(lstAvatarBase)
		  txtRicerca.Text = ""
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events lstAvatarBase
	#tag Event
		Sub Opening()
		  me.Heading(0) = "ID"
		  me.Heading(1) = "Name"
...
#tag Events cmdInserisci
	#tag Event
		Sub Pressed()
		  if Session.wdInserisciAvatarBaseAperto = False Then
		    dim t As new wdInserisciAvatarBase
		    t.lstwdAvatarBase = lstAvatarBase
		    t.Show
		  end if
		End Sub
	#tag EndEvent
...
#tag Events cmdModifica
	#tag Event
		Sub Pressed()
		  if Session.wdModificaAvatarBaseAperto = False Then
		    dim t As new wdModificaAvatarBase
		    t.lstwdAvatarBase = lstAvatarBase
		    t.Show
		  end if
		End Sub
...
#tag Events cmdElimina
	#tag Event
		Sub Pressed()
		  if lstAvatarBase.ListIndex = -1 then
		  ...
		  dim sql As String
		  sql = "DELETE FROM basic_avatar WHERE idavatar="+lstAvatarBase.RowTag(lstAvatarBase.ListIndex)
		  db.SQLExecute(sql)
		  if db.Error then
		    MsgBox ErroreDatabase
		    Exit Sub
		  ...
		  MsgBox "The basic avatar has been deleted."
		  AggiornalstAvatarBase(lstAvatarBase)
		End Sub
	#tag EndEvent
#tag EndEvents
