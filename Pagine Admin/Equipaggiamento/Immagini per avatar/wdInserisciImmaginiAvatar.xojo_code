#tag WebPage
Begin WebDialog wdInserisciImmaginiAvatar
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   462
...
#tag WindowCode
	#tag Event
		Sub Close()
		  if Session.Available Then
		    Session.wdInserisciImmaginiAvatarAperto = False
		  end if
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub Open()
		  if Session.Available then
		    Session.wdInserisciImmaginiAvatarAperto = True
		  end if
	...
	#tag Event
		Sub RichiestaSalvataggioDati(jsonPosizioni As String)
		  ...
		  if connettiDB Then
		    ...
		    MsgBox "The images have been inserted with successful."
		...
		Sub Opening()
		  ...
		  if connettiDB then
		    ...
		    while not rs.EOF
		      ...
		      me.RowTag(i)=rs.IdxField(1).Value
		      i = i + 1
		      rs.MoveNext
		    wend
		...
		Sub UploadFinished(Files() As WebUploadedFile)
		  ...
		  For Each file As WebUploadedFile In Files
		    Try
		      ...
		      sql = sql + str(idEquipaggiamento) + ",'" + uploadFolder.Name + "','" + file.Name + "'," + pmAvatar.RowTag(pmAvatar.ListIndex) + ")"
		      if connettiDB then
		        db.SQLExecute(sql)
		        if db.Error then
		          pictureFile = uploadFolder.Child(file.Name)
