...
#tag WindowCode
	#tag Method, Flags = &h0
		Sub AggiornalstUtenti()
		  dim sql As String
		  sql = "SELECT idmobster, name, HG FROM mobster"
...
#tag Events cmdInvia
	#tag Event
		Sub Pressed()
		  //Controlli
		  if txtHG.Text = "" then
		    MsgBox "Insert a valid value for HGs."
		    txtHG.SetFocus
		    Exit sub
		  end if
		  
		  if IsNumeric(txtHG.Text) = False then
		    MsgBox "HGs value must be a number."
		    txtHG.SetFocus
		    Exit sub
		  end if
		  
		  dim i, nHG As Integer
		  dim sql As String
		  if cbInviaATutti.Value = False Then
		    for i=0 to lstUtentiSelezionati.RowCount-1
		      //Numero HG aggiornato
		      nHG = Val(lstUtentiSelezionati.Cell(i,2)) + Val(txtHG.Text)
		      //Insertisco nel db
		      sql = "UPDATE mobster SET HG=" + Str(nHG) + " WHERE idmobster="+lstUtentiSelezionati.RowTag(i)
		  ...
		  Else
		    ...
		    sql = "SELECT idmobster, HG FROM mobster"
		    dim rs As RecordSet
		    rs = db.SQLSelect(sql)
		    //Controllo errori
		    if db.Error Then
		    ...
		    for i = 0 to rs.RecordCount-1
		      sql = "UPDATE mobster SET HG=" + Str(rs.IdxField(2).Value+val(txtHG.Text)) + " WHERE idmobster="+rs.IdxField(1).StringValue
		      //Eseguo Update e controllo per errori
		      if connettiDB Then
		        db.SQLExecute(sql)
		        if db.Error Then
